...
def norm_xcorr2d(patch, template):
    ...
    for i in range(len(patch)):
        for j in range (len(patch[0])):
            patch[i][j]=patch[i][j]-i_avg
            template[i][j]=template[i][j]-p_avg
            
    dot_pro=0       
    for i in range(len(patch)):
        for j in range(len(patch[0])):
            dot_pro+=patch[i][j]*template[i][j]
            
    vec_img=0 
    vec_patch=0       
    for i in range(len(patch)):
        for j in range(len(patch[0])):
            vec_img+=patch[i][j]*patch[i][j]
            vec_patch+=template[i][j]*template[i][j]
            
            
    value=dot_pro/((vec_img**0.5)*(vec_patch**0.5))
...
def match(img, template):
    ...
    for k in range(len(img)-len(template)+1):    
        for l in range(len(img[0])-len(template[0])+1):
        
            for i in range(len(template)):
                for j in range (len(template[0])):
                    temp.append(img[i+k][j+l])
                new_patch.append(temp)
                temp=[]
            
            temp_ncc=norm_xcorr2d(new_patch,template)
            if(temp_ncc>max_value):
                max_value=temp_ncc
                x=k
                y=l
                #req_ncc_mat=new_patch_dup
            new_patch=[]
